...
static inline int get_obj (lua_State *L)
{
  ...
  tkpdf_obj_t *tkobj = lua_newuserdata(L, sizeof(tkpdf_obj_t));
...
static inline int get_page (lua_State *L)
{
  ...
  if (!page)
    return 0;
  tkpdf_page_t *tkpage = lua_newuserdata(L, sizeof(tkpdf_page_t));
  luaL_getmetatable(L, MT_PAGE);
  lua_setmetatable(L, -2);
  tkpage->page = page;
  return 1;
}
...
static inline int get_page_dict (lua_State *L)
{
  ...
  if (!dict)
    return 0;
  tkpdf_dict_t *tkdict = lua_newuserdata(L, sizeof(tkpdf_dict_t));
  luaL_getmetatable(L, MT_DICT);
  lua_setmetatable(L, -2);
  tkdict->dict = dict;
  return 1;
}
...
static inline int get_dict_dict (lua_State *L)
{
  ...
  if (!dict)
    return 0;
  tkpdf_dict_t *tkdict0 = lua_newuserdata(L, sizeof(tkpdf_dict_t));
  luaL_getmetatable(L, MT_DICT);
  lua_setmetatable(L, -2);
  tkdict0->dict = dict;
  return 1;
...
static inline int get_dict_array (lua_State *L)
{
  ...
  tkpdf_array_t *tkarray = lua_newuserdata(L, sizeof(tkpdf_array_t));
  luaL_getmetatable(L, MT_ARRAY);
  lua_setmetatable(L, -2);
  tkarray->array = array;
  return 1;
}
...
static inline int get_array_array (lua_State *L)
{
  ...
  tkpdf_array_t *tkarray0 = lua_newuserdata(L, sizeof(tkpdf_array_t));
  luaL_getmetatable(L, MT_ARRAY);
  lua_setmetatable(L, -2);
...
static inline int get_obj_stream (lua_State *L)
{
  ...
  if (!stream)
    return 0;
  tkpdf_stream_t *tkstream = lua_newuserdata(L, sizeof(tkpdf_stream_t));
  luaL_getmetatable(L, MT_STREAM);
  lua_setmetatable(L, -2);
